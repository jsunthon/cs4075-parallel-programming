/*
Build derived database from a collection of contiguous elements. Use it to rewrite 
print vector and print read vector.
 */
...
void Read_vector(int x[], int y[], int local_vec1[], int local_vec2[], int local_n,
	int my_rank, int comm_sz, MPI_Comm comm, MPI_Datatype new_mpi_t_p);
...
void Print_vector_sum(int local_vector_sum[], int local_n, int n, 
	int my_rank, MPI_Comm comm, MPI_Datatype new_mpi_t_p);
...
int main(void) {
	...
	MPI_Type_contiguous(local_n, MPI_INT, &new_mpi_t_p);
	MPI_Type_commit(&new_mpi_t_p);

	x = malloc(local_n * comm_sz * sizeof(int));
	y = malloc(local_n * comm_sz * sizeof(int));
	Read_vector(x, y, local_vec1, local_vec2, local_n, my_rank, comm_sz, MPI_COMM_WORLD, new_mpi_t_p);
	...
	Print_vector_sum(local_vector_sum, local_n, n, my_rank, MPI_COMM_WORLD, new_mpi_t_p);
...
void Read_vector(int x[], int y[], int local_vec1[], int local_vec2[], int local_n,
	...
	if (my_rank == 0) {
		...
		MPI_Scatter(x, 1, new_mpi_t_p, local_vec1, 1,
			new_mpi_t_p, 0, comm);
		...
		MPI_Scatter(y, 1, new_mpi_t_p, local_vec2, 1,
			new_mpi_t_p, 0, comm);
	} else {
		MPI_Scatter(x, 1, new_mpi_t_p, local_vec1, 1,
			new_mpi_t_p, 0, comm);
...
void Print_vector_sum(int local_vector_sum[], int local_n, int n,
 int my_rank, MPI_Comm comm, MPI_Datatype new_mpi_t_p) {
	...
	if (my_rank == 0) {
		...
		MPI_Gather(local_vector_sum, 1, new_mpi_t_p, vector_sum,
			1, new_mpi_t_p, 0, comm);
		printf("\nVector sum:\n");
